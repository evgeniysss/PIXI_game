...
class FallingShape {
    ...
    GetPolygonVertices(sides, size) {
        let centerAng = 2*Math.PI / sides;
        let vertices = new Array();
        for (let i=0 ; i<sides ; i++) {
            let ang = i*centerAng;
            let vx = Math.round(size*Math.cos(ang));
            let vy = Math.round(size*Math.sin(ang)); 
            vertices.push(vx, vy);
        }
        return vertices;
    }
...
const mainContainer = document.getElementById('render-zone');
...
let timeFromLastShapeCreation = 0;
...
app.ticker.add(function(delta) {
    ...
    for (childId in app.stage.children) {
        let thisChild = app.stage.children[childId];
...
function createNewShape(options) {
    ...
    let newShape = new FallingShape(options.x, options.y, 0, Math.floor(Math.random()*7), '0x'+Math.random().toString(16).slice(2,8), sceneOptions.shapesSizes, Math.random()*Math.PI*2);
